...
async def samsung(email, client, out):
    ...
    try:
        ...
        token_match = re.search(r'sJSESSIONID.*?"([^"]+)"', req.text)
        csrf_match = re.search(r"\{'token'\s*:\s*'([^']+)'\}", req.text)
        if token_match is None or csrf_match is None:
            raise ValueError('samsung signup page changed')

        token = token_match.group(1)
        crsf = csrf_match.group(1)
    ...
    if response.status_code == 200:
        if isinstance(data, dict) and "rtnCd" in data.keys() and "INAPPROPRIATE_CHARACTERS" not in response.text and "accounts aren't supported." not in response.text:
...
async def get_phone_number(email, client, cookies, headers):
    ...
    try:
        ...
        phone_number = None
        if isinstance(data, dict) and data.get('rtnCd') == 'NEXT':
